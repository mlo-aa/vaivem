...
**Env (web).** `AUTH_SECRET` (required in production; ephemeral random secret in local if unset — set it in `apps/web/.env.local` so Edge middleware and Node API share the same key); optional `RESEND_API_KEY` + `EMAIL_FROM` for real email (omit for console/devMode codes); Etherfuse + Stellar sponsor vars as before; optional `ETHERFUSE_MXN_BANK_ACCOUNT_ID` / `ETHERFUSE_CRYPTO_WALLET_ID` (otherwise resolved from the org).
